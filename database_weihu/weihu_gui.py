...
from TableView_test import Ui_dataBaseForm
...
class MyMain(QMainWindow,Ui_dataBaseForm):
    ...
    def dbWrit(self):
        ...
        if len(self.text) != 2:
            ...
            # try:
            MyMain.cursor.execute( "INSERT INTO {0} (date,line1,line2) VALUES((SELECT now()),'{1}','{2}')"
                                   .format(self.dbName, self.text[0],self.text[1]))#, self.text[2], '{2}' , '{3}'
            MyMain.conn.commit()
            # except Exception as exc:
            # self.alarmMessageBox(str(exc))
            # MyMain.conn.commit()
    ...
    def rogatoryCondition(self):
        ...
        self.dbName = self.db_create_edit.text()
        self.select = "SELECT date, id ,line1,line2 FROM {0} WHERE date > '{1}' and  date < '{2}'".format(#,line2,line3
            self.dbName,
            self.startTime,
            self.endTime)
        return self.select
