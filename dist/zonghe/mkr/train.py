...
def train(args, data, show_loss, show_topk):
    ...
    if args.dataset=="xingce":
        ckpt=tf.train.get_checkpoint_state(resource_path("model"))
    else:
        ckpt = tf.train.get_checkpoint_state(resource_path("model_shenlun"))
    ...
    with tf.Session() as sess:
        ...
        for step in range(args.n_epochs):
            ...
            if step % args.kge_interval == 0:
                ...
                while start < kg.shape[0]:
                    ...
                    if show_loss:
                        print(rmse)
                    if args.dataset=="xingce":
                        saver.save(sess,save_path=resource_path("model/model.ckpt"))
                    else:
                        saver.save(sess, save_path=resource_path("model_shenlun/model.ckpt"))
            #输出本次的训练结果
            ...
            if show_topk:
                ...
                for i in f1:
                    print('%.4f' % i, end=' ')
        predict = model.predict(sess, get_feed_dict_for_predict(model, predict_data, 0, predict_data.shape[0]))
        predict=sorted(predict,key=lambda e:e[1],reverse=True)
        res_recom=[]
        for item in predict:
            res_recom.append(item[0])
        if args.dataset=="xingce":
            f1=open(resource_path("result.pkl"), "wb")
            pickle.dump(res_recom,f1)
            f1.close()
        else:
            f1 = open(resource_path("result_shenlun.pkl"), "wb")
            pickle.dump(res_recom, f1)
            f1.close()
        print(predict)
...
def get_feed_dict_for_rs(model, data, start, end):
    ...
    return feed_dict
def get_feed_dict_for_predict(model, data, start, end):
    feed_dict = {model.user_indices: data[start:end, 0],
                 model.item_indices: data[start:end, 1],
                 model.labels: data[start:end, 2],
                 model.head_indices: data[start:end, 1]}
...
if __name__ == '__main__':
     ...
     import argparse
     data_loader = __import__("data_loader")
